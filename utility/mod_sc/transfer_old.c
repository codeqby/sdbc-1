...
errret:
	...
	while(t_cont==PACK_CONTINUE) {
		...
		if(i) {
			release_SC_connect(&gp->server,srvp->TCB_no,srvp->poolno);
			sprintf(tmp,"PACK_CONTINUE tid=%lu,TCB:%d,"
				  "recv from client errno=%d,%s",
				      srvp->tid,srvp->TCB_no,errno,strerror(errno));
			goto clierr;
		}
...
err:
	...
	ShowLog(3,"%s:TCB:%d,tid=%lu,Recv from server %s PROTO_NUM=0X%04X,"
		  "ERRNO1=%d,ERRNO2=%d,PKG_LEN=%d,T_LEN=%d,t_cont=%08X,USEC=%lld",
			...
			gp->server->Host,proto_num, NetHead->ERRNO1, NetHead->ERRNO2,
...
//下一个珠子是:from_client,下一个链子是：主任务队列
		i=set_event(srvp->TCB_no,client->Socket,from_client,60);
		if(i) {
			ShowLog(1,"%s:TCB:%d set_event error %d",__FUNCTION__,
				srvp->TCB_no,i);
			return -1;
...
unsigned int t_cont;
	...
	i=SendPack(server,NetHead);
	ShowLog(3,"%s:TCB:%d,tid=%lu,Send to serv proto_num=0X%04X,t_cont=%d,SendPack ret=%d,USEC=%lld",
		__FUNCTION__,srvp->TCB_no,srvp->tid,NetHead->PROTO_NUM,t_cont,i,now_usec());
	if(i){
		NetHead->ERRNO1=errno;
		NetHead->ERRNO2=i;
		sprintf(tmp,"Send to %s fail ret=%d,errno=%d,%s",
...
errret:
	...
	while(t_cont==PACK_CONTINUE) {
		...
		if(i) {
			release_SC_connect(&gp->server,srvp->TCB_no,srvp->poolno);
			ShowLog(1,"%s:PACK_CONTINUE tid=%lu,TCB:%d,"
				  "recv from client ret=%d,errno=%d,%s",__FUNCTION__,
				      srvp->tid,srvp->TCB_no,errno,strerror(errno));
			return -1;
		}
//ShowLog(5,"Transfer c->s CONTINUE PROTO_NUM=%04X",NetHead->PROTO_NUM);
