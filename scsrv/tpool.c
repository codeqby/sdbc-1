...
TCB *enp;
	...
	enp=(*rp)->next;//ÕÒµ½¶ÓÍ·
	if(!enp->next) {
		ShowLog(1,"%s:TCB=%d,´í£¬Î´ÔÚ¶ÓÁÐÖÐ£¡",__FUNCTION__,enp->sv.TCB_no);
		return -2;
	}
...
static void rdy_add(TCB *en)
{
	if(en->next) { //TCB ÒÑ¾­ÔÚ¶ÓÁÐÖÐ
		ShowLog(1,"%s:TCB:%d ÒÑ¾­ÔÚ¶ÓÁÐÖÐ",__FUNCTION__,en->sv.TCB_no);
		return;
	}
	if(!rpool.queue) {
		rpool.queue=en;
...
//Ð­ÉÌÃÜÔ¿
		if(task->status==-1) {
			...
			if(ret<0) {
				if(ret!=SYSERR) 
					ShowLog(1,"%s:tid=%lX,TCB:%d,Ð­ÉÌÃÜÔ¿Ê§°Ü!,ret=%d",__FUNCTION__,
...
//½¡¿µ¼ì²éÖÜÆÚ
		...
		ret=do_epoll(task,EPOLL_CTL_ADD);//ÈÎÎñ½»¸øÆäËûÏß³Ì×ö
	}
